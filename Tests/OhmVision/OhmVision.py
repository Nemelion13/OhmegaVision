...
# [Teinte min, Saturation min, Valeur min], [Max...], "NOM", Valeur, (Couleur BGR pour l'affichage)
COLOUR_BOUNDS = [
...
def init(debug_mode):
    ...
    # Assure-toi que le chemin vers le fichier XML est correct par rapport à l'endroit où tu lances le script
    cascade_path = os.path.join(tPath, "cascade", "haarcascade_resistors_0.xml")
    rectCascade = cv2.CascadeClassifier(cascade_path)
    
    if rectCascade.empty():
        print(f"Erreur : Impossible de charger le classificateur depuis {cascade_path}")
...
def printResult(sortedBands, liveimg, resPos):
    ...
    # Calcule uniquement si on a trouvé 3, 4 ou 5 bandes
    if len(sortedBands) in [3, 4, 5]:
        ...
        cv2.putText(liveimg, f"{intVal} OHMS", (x + w + 10, y), FONT, 1, (255, 255, 255), 2, cv2.LINE_AA)
    else:
        # Erreur de lecture : rectangle rouge
        cv2.rectangle(liveimg, (x, y), (x + w, y + h), (0, 0, 255), 2)

def findResistors(liveimg, rectCascade):
    # Correction de variable : utilisation de 'liveimg' au lieu de 'cliveimg'
...
def findBands(resistorInfo, debug_mode):
    if debug_mode:
    ...
    bandsPos = []
    checkColours = COLOUR_BOUNDS[0:1] if debug_mode else COLOUR_BOUNDS
    ...
    for clr in checkColours:
        ...
        # CORRECTION OpenCV 4 : cv2.findContours ne retourne que 2 variables
        contours, hierarchy = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        
        # CORRECTION Tuple : conversion en liste
